...
- graft subnetworks to backbone (see google groups convo w Rowan Schley) []
    - decide what species to include along backbone (rogue ones) []
        0. Rogue species: S. corymbifer, S. nematosepalus, S. smilax (2), C. tessmannii
        1. Eucentropogonid:
        2. Brevilimbatid:
        3. Andinus:
        4. Tupaeformis:
        5. Giganteus + burmeisterids:
        6. Odontosepalus + colombianids:
        7. Correoides + nobilis:
        8. Furax + actinothrix + S. asplundii
        9. Peruvianids:
        10. Umbellatus:
...
@sync @distributed for (clade, h) in clades_h_list[27:40]
...
## Visualizing best h-vals

Get nets and scores from logs, store in a dict:
```{julia}
vals = [0, 1, 2, 3]
subclade_scores = Dict{String, Vector}()
...
for clade in subnetwork_names
    ...
    for h in vals
        ...
        filename = "output/snaq/$clade-hmax$h.out"
        open(filename) do f
    ...
    subclade_scores[clade] = [clade_scores, clade_nets]
end
```

```{julia}
for (k,v) in subclade_scores
    #add title with subclade name (k)
    #add labels for x and y axis
    plot(collect(0:3), v[1])
    savefig("$k-hvals.pdf")
end
```
...
```{julia}
supported_nets = []
#second item is list of networks
#last value is best supported h (+1 because we have 0 in there)
push!(supported_nets, subclade_scores["actinothrix"][2][2]);
push!(supported_nets, subclade_scores["andinus"][2][2]);
push!(supported_nets, subclade_scores["brevilimbatid"][2][4]);
push!(supported_nets, subclade_scores["correoides_nobilis"][2][3]);
push!(supported_nets, subclade_scores["eucentropogonid"][2][3]);
push!(supported_nets, subclade_scores["giganteus_burmeisterid"][2][2]);
push!(supported_nets, subclade_scores["odontosepalus_colombianid"][2][2]);
push!(supported_nets, subclade_scores["peruvianid"][2][2]);
push!(supported_nets, subclade_scores["tupaeformis"][2][2]);
push!(supported_nets, subclade_scores["umbellatus"][2][2]);

supported_nets

for net in supported_nets
    try
        rootatnode!(net, "Lysipomia_sp2")
    catch e
    end
end

#fixit: naming convention for this
PhyloPlots.plot(supported_nets[1])
PhyloPlots.plot(supported_nets[2])
PhyloPlots.plot(supported_nets[5])
PhyloPlots.plot(supported_nets[6])
```