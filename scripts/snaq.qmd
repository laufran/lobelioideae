...
```{julia}
samples = DataFrame(CSV.File("data/sampling_csvs/samples2024.csv",stringtype=String)) #load df from CSV
...
for group in subnetwork_groups
    ...
    snaq!(cladetree, cftable, hmax=0, filename="output/snaq/$cladestring-net0", seed=1234)
    idx += 1
end
```