...
```{julia}
samples = DataFrame(CSV.File("samples2024.csv",stringtype=String)) #load df from CSV
...
sptree = readTopology("data/newgts/newick.tre")
...
for group in subnetwork_groups
    ...
    #fixit: give different starting seed each time
    #fixit: decide on search parameters
    snaq!(cladetree, cftable, hmax=0, filename="output/$cladestring-net0", seed=1234)
    idx += 1
end
```