...
```{julia}
using CSV, DataFrames, PhyloNetworks, StatsBase, Random, PhyloPlots
...
#hard code correct hvals from qgof & looking at hvals 
#this is a dictionary that should NOT be copied to other docs
#setting all hvals above 0 to calibrate nets for each subclade
#then also calibrating trees for all subclades as well
...
## Load in subnetworks - those with smilax
```{julia}
subclade_scores_smilax = Dict{String, Vector}()

for clade in ["peruvianid", "odontosepalus_colombianid", "giganteus_burmeisterid"]
    clade_scores = []
    clade_nets = []
...
helper functions:
```{julia}
function getsupportednet(clade::AbstractString, nets_to_hvals::Dict, subclade_scores::Dict)
    hval_index = nets_to_hvals[clade] + 1
    subclade_scores[clade][2][hval_index]
end

function gettree(clade::AbstractString, subclade_scores::Dict)
    subclade_scores[clade][2][1]
end
```
...
```{julia}
#run for all clades (nets & trees) without smilax:
for clade in subnetwork_names
    @eval $(Symbol(clade * "_tree")) = gettree($clade, subclade_scores_nosmilax)
    @eval $(Symbol(clade * "_net")) = getsupportednet($clade, nets_to_hvals_nosmilax, subclade_scores_nosmilax)
    @eval $(Symbol(clade * "_gts")) = prunegenetrees(genetrees, $clade, subnetwork_groups, subnetwork_names)
end

#with smilax: just run odontosepalus_colombianid, as supported by qgof
clade = "odontosepalus_colombianid"
@eval $(Symbol(clade * "_smilax_tree")) = gettree($clade, subclade_scores_smilax)
@eval $(Symbol(clade * "_smilax_net")) = getsupportednet($clade, nets_to_hvals_smilax, subclade_scores_smilax)
#of note: subclade labels loaded in as part of snaq_setup.jl does NOT include smilax
...
@eval $(Symbol(clade * "_smilax_gts")) = prunegenetrees(genetrees, $clade, subnetwork_groups, subnetwork_names)
```
...
for clade in subnetwork_names
    ...
    tree_sym = Symbol(clade * "_tree")
    net_sym  = Symbol(clade * "_net")
    calnet_sym  = Symbol(clade * "_net_calibrated")
    caltree_sym = Symbol(clade * "_tree_calibrated")
    filename_net  = "output/snaq-withoutsmilax/" * clade * "_calibratednet.tre"
    filename_tree = "output/snaq-withoutsmilax/" * clade * "_calibratedtree.tre"

    @eval begin
        $calnet_sym  = calibratenet($gts_sym, $clade, $net_sym)
        $caltree_sym = calibratenet($gts_sym, $clade, $tree_sym)
        rootatnode!($caltree_sym, "Lysipomia_pumila")
        try rootatnode!($calnet_sym, "Lysipomia_pumila")
        catch e
          @show "$clade cannot be rooted"
        end
        #will say that odontosepalus_colombianid cannot be rooted, that's fine,
          #we're using the smilax version anyway
        writenewick($calnet_sym, $filename_net)
        writenewick($caltree_sym, $filename_tree)
    end
end
```

calibrate and write out newick to `output/snaq-withsmilax/` just for "odontosepalus_colombianid":
```{julia}
clade = "odontosepalus_colombianid"
gts_sym  = Symbol(clade * "_gts")
tree_sym = Symbol(clade * "_smilax_tree")
net_sym  = Symbol(clade * "_smilax_net")
calnet_sym  = Symbol(clade * "_net_calibrated")
caltree_sym = Symbol(clade * "_tree_calibrated")
filename_net  = "output/snaq-withsmilax/" * clade * "_calibratednet.tre"
filename_tree = "output/snaq-withsmilax/" * clade * "_calibratedtree.tre"
...
# Combine subclades into artificial star tree

Need to remove outgroup from each subnetwork, as they all have it:

```{julia}
#this should be fine for odontosepalus_colombianid
  # --> I rewrite the odontosepalus_colombianid variable 
nets_to_finalhvals =  Dict("actinothrix" => 1, 
                        "andinus" => 1, 
                        "brevilimbatid" => 0,
...
calibrated_nets = []
for clade in subnetwork_names
  if nets_to_finalhvals[clade] == 0
    caltree_sym = Symbol(clade * "_tree_calibrated")
    push!(calibrated_nets, eval(caltree_sym))
  else
    calnet_sym  = Symbol(clade * "_net_calibrated")
    push!(calibrated_nets, eval(calnet_sym))
  end
end
...
plot(supernet); #can see that not all clades have hyb. events --> good! some *are* trees.
plot(supernet, useedgelength=true); #yep superlong artificial branch lengths
