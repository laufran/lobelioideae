...
```{julia}
aic(s1_poll) #94.33747997755411 --> much better!
aic(s2_poll) #252.2718658578484
aic(s1_fruit) #49.439072090658534 --> much better!
aic(s2_fruit) #87.34871719969516
```

we want the model with the lowest AIC --> in this case both s1 (ERSM) models were best.

julia> s1_poll = fitdiscrete(supertree, :ERSM, species, select(filtered_nosicklebill, :trait); optimizeQ=true)
PhyloTraits.StatisticalSubstitutionModel:
Equal Rates Substitution Model with k=2,
  all rates equal to α=1.94502.
  rate matrix Q:
       hummingbird     bat
  hummingbird       *  1.9450
       bat  1.9450       *
...
data:
  92 species
  1 trait
log-likelihood: -46.16874

julia> s1_fruit = fitdiscrete(supertree, :ERSM, species, select(filtered_fruit, :trait); optimizeQ=true)
PhyloTraits.StatisticalSubstitutionModel:
Equal Rates Substitution Model with k=2,
  all rates equal to α=0.5151.
  rate matrix Q:
           capsule   berry
   capsule       *  0.5151
     berry  0.5151       *
...
data:
  92 species
  1 trait
log-likelihood: -23.71954

```{julia}
s1_poll_rate = s1_poll.model.rate[1] #1.945
s1_fruit_rate = s1_fruit.model.rate[1] #0.5151
```
...
for (trait,cladelist) in traits_to_clades
    for clade in cladelist
        ...
        PhyloNetworks.resetnodenumbers!(cladetree)

        #rotate nodes so as to untangle:
        #note: doing this BEFORE doing ancestralstatereconstruction
        #that way nodes match up in ASR model and net
        if clade == "tupaeformis"
            for n in [19, 20, 21, 22]; rotate!(cladetree, n); end
        elseif clade == "actinothrix"
            rotate!(cladetree, 9)
        elseif clade == "andinus"
            for n in [12, 13, 16]; rotate!(cladetree, n); end
        elseif clade == "umbellatus"
            for n in [7, 8]; rotate!(cladetree, n); end
        elseif clade == "odontosepalus_colombianid"
            #if excluding smilax:
            #for n in [21, 22, 16, 17]; rotate!(cladetree, n); end
            for n in [15, 23, 14, 18]; rotate!(cladetree, n); end
        end
